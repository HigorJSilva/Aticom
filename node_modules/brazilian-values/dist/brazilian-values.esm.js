...
var parsePiecesToDate = function parsePiecesToDate(pieces) {
  return new Date(pieces.year, pieces.month, pieces.date);
};
...
var comparePiecesToDate = function comparePiecesToDate(pieces, instance) {
  ...
  return instance.getDate() === pieces.date && instance.getMonth() === pieces.month && instance.getFullYear() === pieces.year;
};
...
var parseToDate = function parseToDate(value) {
  if (!DATE_PATTERN.test(value)) throw new Error("Value \"" + value + "\" does not match format.");
...
var DATE_PATTERN$1 = /^\d{2}\/\d{2}\/\d{4}$/;
/**
...
var isDate = function isDate(value) {
  return DATE_PATTERN$1.test(value) && comparePiecesToDate(mapToPieces(value));
};
...
var capitalizeWord = function capitalizeWord(word) {
  return word[0].toLocaleUpperCase() + word.substr(1).toLocaleLowerCase();
};
...
var splitIntoWords = function splitIntoWords(value) {
  var text = normalizeWhiteSpaces(value);
...
var formatToCapitalized = function formatToCapitalized(value, _a) {
  ...
  return splitIntoWords(value).map(function (word, index) {
    var lowerCaseWord = word.toLocaleLowerCase();
    if (index > 0 && wordsToKeepLowerCase.indexOf(lowerCaseWord) !== -1) return lowerCaseWord;
    if (wordsToKeepUpperCase.indexOf(lowerCaseWord) !== -1) return word.toLocaleUpperCase();
...
export { formatToBRL, formatToCEP, formatToCNPJ, formatToCPF, formatToCapitalized, formatToDate, formatToDateTime, formatToList, formatToNumber, formatToPhone, formatToRG, isCEP, isCNPJ, isCPF, isDDD, isDate, isPhone, parseToArray, parseToDate, parseToNumber };
